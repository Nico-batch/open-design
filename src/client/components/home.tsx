    ...
    <div class="min-h-full bg-zinc-950">
      ...
      <div class="bg-zinc-900 border-b border-zinc-800">
        <div class="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          <div>
            <h1 class="text-lg font-bold text-zinc-100 m-0">My Designs</h1>
      ...
      <div class="max-w-6xl mx-auto px-6 py-6">
        ...
        {templates.length > 0 && (
          <div class="mb-8">
            <div class="flex items-center gap-2 mb-3">
              ...
              <h2 class="text-sm font-semibold text-zinc-200 m-0">Start from a template</h2>
            </div>
        ...
        {designs.length === 0 ? (
          <div class="text-center py-20">
            <div class="w-16 h-16 rounded-2xl bg-zinc-700 flex items-center justify-center mx-auto mb-4">
              <Plus size={24} class="text-zinc-400" />
            </div>
            <p class="text-sm text-zinc-400 mb-1">No designs yet</p>
          ...
          <>
            <h2 class="text-sm font-semibold text-zinc-200 mb-3 m-0">Recent designs</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {designs.map((d) => (
                <div
                  key={d.id}
                  class="bg-zinc-800 rounded-xl border border-zinc-800 overflow-hidden cursor-pointer transition-all hover:border-[#6366f1] hover:shadow-md group"
                ...
                >
                  ...
                  <div class="aspect-[4/3] bg-zinc-800 flex items-center justify-center">
                    {d.thumbnail_url ? (
                      ...
                      <div class="text-zinc-200 text-[10px] font-medium">
                        {d.width} x {d.height}
                      </div>
                    )}
                  </div>
                  ...
                  <div class="p-3">
                    {editingId === d.id ? (
                      <input
                        class="w-full bg-zinc-800 border border-[#6366f1] rounded text-zinc-100 text-xs px-2 py-1 outline-none"
                        value={editName}
                      ...
                      <div class="flex items-start justify-between">
                        <div class="min-w-0 flex-1">
                          <p class="text-xs font-semibold text-zinc-200 truncate m-0">
                            {d.name}
                          </p>
                        ...
                        <div class="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity ml-2 shrink-0">
                          <button
                            class="p-1 rounded text-zinc-400 bg-transparent border-none cursor-pointer hover:text-zinc-200 transition-colors"
